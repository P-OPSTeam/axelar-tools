...
if [[ "$reset" =~ "false" ]]; 
then
    sudo ./join/join-testnet-with-binaries.sh --axelar-core ${CORE_VERSION} &>> testnet.log
else
    sudo ./join/join-testnet-with-binaries.sh --axelar-core ${CORE_VERSION} --reset-chain &>> testnet.log
fi
